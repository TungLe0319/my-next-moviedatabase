...
export default function MoviePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const mainActorData = await movieService.fetchMovieMainActors(
        slug as string
      );
      ...
      if (data && mainActorData) {
        ...
        setMain_actors(mainActorData.results.cast.edges);
      }
    };
  ...
  return (
    <main className="   bg-gradient-to-br from-gray-950 via-gray-800 to-emerald-400 min-h-screen flex-col items-center justify-between p-44">
        ...
        <Box className="  flex  pb-5 ">
          ...
          <div className=" w-2/3   flex items-center ">{movie?.plot.plotText.plainText}</div>
        </Box>
...
export async function getServerSideProps(context: { params: { slug: any; }; }) {
  const { slug } = context.params; // Access the slug from the URL params
